...
if st.session_state.pa_current_product:
    ...
    if detail_df.empty:
        ...
        if st.button("🤖 生成AI诊断报告", key="pa_ai_diagnosis"):
            ...
            {trend_desc}
            同类商品平均退货率: {filtered[filtered['categories']==category]['退货率'].mean() if category!='未知' else filtered['退货率'].mean():.1f}%
            """
            prompt = """
            ...
            with st.spinner("AI 正在分析..."):
                diagnosis = get_ai_summary(prompt, context, "Qwen2.5-7B")
            st.session_state.pa_diagnosis_result = diagnosis
            st.rerun()
        
        if st.session_state.pa_diagnosis_result:
            st.markdown(f"""
            <div class="diagnosis-box">
                <div style="font-weight:600; margin-bottom:8px;">📋 诊断结论</div>
                <div>{st.session_state.pa_diagnosis_result}</div>
            </div>
            """, unsafe_allow_html=True)
