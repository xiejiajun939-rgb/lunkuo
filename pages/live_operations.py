...
layout_navigation_column, content_column = st.columns([0.18, 0.82], gap="large")
content_column.__enter__()
...
.main div[data-testid="stColumn"]:has(.live-workspace-nav){position:sticky!important;top:20px!important;align-self:flex-start!important;max-height:calc(100vh - 40px)!important;overflow:visible!important}
.live-workspace-nav{position:relative;width:100%;max-height:calc(100vh - 40px);box-sizing:border-box;padding:20px 14px 16px;background:linear-gradient(180deg,#061a2e 0%,#08243b 55%,#071c30 100%);border:1px solid rgba(89,211,239,.18);border-radius:14px;box-shadow:0 8px 24px rgba(4,22,39,.10);color:#eaf7fb;overflow-x:hidden;overflow-y:auto;scrollbar-width:thin;scrollbar-color:rgba(77,211,235,.38) transparent}
...
@media(max-width:900px){.main .block-container{padding:18px 18px 40px!important}.main div[data-testid="stColumn"]:has(.live-workspace-nav){position:relative!important;top:auto!important;max-height:none!important}.live-workspace-nav{max-height:none;margin:0 0 16px;overflow:visible}.live-workspace-nav__item{display:inline-block;width:calc(50% - 6px);box-sizing:border-box;vertical-align:top}}
...
content_column.__exit__(None, None, None)
with layout_navigation_column:
    st.markdown(
